...
(function () {
  ...
  // Builds the markup for one result card. `tool` comes from the trusted
  // window.TOOLS registry (not user input), so innerHTML is safe here.
  DT.cardHTML = function (tool) {
    const label = tool.category.charAt(0).toUpperCase() + tool.category.slice(1);
    return (
      '<a class="card" href="' + tool.href + '">' +
      '<div class="card__icon mono" aria-hidden="true">' + tool.icon + "</div>" +
  ...
  function renderSearch() {
    ...
    if (matches.length) {
      grid.innerHTML = matches.map(DT.cardHTML).join("");
      noResults.classList.remove("is-visible");
    } else {
      grid.innerHTML = "";
